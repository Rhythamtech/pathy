...
const Sessions = () => {
  ...
  useEffect(() => {
    if (hydrated && sessionId && selectedEndpoint && agentId) {
      getSession({ entityType: 'agent', agentId, dbId }, sessionId)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hydrated, sessionId, selectedEndpoint, agentId, dbId])
  ...
  if (isSessionsLoading || isEndpointLoading) {
    return (
      <div className="w-full">
        <div className="mb-2 text-xs font-medium uppercase text-accent">Sessions</div>
  ...
  return (
      <div className="w-full">
      <div className="mb-2 w-full text-xs font-medium uppercase text-accent">Sessions</div>
      <div
        className={`h-[calc(100vh-345px)] overflow-y-auto transition-all duration-300 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar]:transition-opacity [&::-webkit-scrollbar]:duration-300 ${
          isScrolling
            ? '[&::-webkit-scrollbar-thumb]:rounded-none [&::-webkit-scrollbar-thumb]:bg-canvas [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:opacity-0'
