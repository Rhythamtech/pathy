...
const HorizontalRule = ({ className, ...props }: HorizontalRuleProps) => (
  <hr
    className={cn(className, 'mx-auto w-48 border-b border-hairline')}
    {...filterProps(props)}
  />
)
...
const InlineCode: FC<PreparedTextProps> = ({ children }) => {
  return (
    <code className="relative whitespace-pre-wrap rounded bg-surface-card p-1">
      {children}
    </code>
  )
}
...
const Img = ({ src, alt }: ImgProps) => {
  ...
  return (
    <div className="w-full max-w-xl">
      {error ? (
        <div className="flex h-40 flex-col items-center justify-center gap-2 rounded-md bg-surface-card text-mute">
          <Paragraph className="text-ink">Image unavailable</Paragraph>
          <Link
            href={srcStr}
            target="_blank"
...
const Table = ({ className, ...props }: TableProps) => (
  <div className="w-full max-w-[560px] overflow-hidden rounded-md border border-hairline">
...
const TableHead = ({ className, ...props }: TableHeaderProps) => (
  <thead
    className={cn(
      className,
      'rounded-md border-b border-hairline bg-transparent p-2 text-left text-sm font-[600]'
    )}
    {...filterProps(props)}
  />
)
...
const TableRow = ({ className, ...props }: TableRowProps) => (
  <tr
    className={cn(className, 'border-b border-hairline last:border-b-0')}
    {...filterProps(props)}
  />
)
